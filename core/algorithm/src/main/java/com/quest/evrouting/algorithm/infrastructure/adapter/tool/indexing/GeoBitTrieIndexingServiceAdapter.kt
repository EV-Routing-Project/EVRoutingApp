package com.quest.evrouting.algorithm.infrastructure.adapter.tool.indexing
...
class GeoBitTrieIndexingServiceAdapter (
    val geohashProvider: GeohashProviderPort,
    ...
    fun getLevelOfRadius(radius: Double, point: Point) : Int {
        for(i in (GeoUtils.MAX_LEVEL downTo GeoUtils.MIN_LEVEL)){
            val latSize = geohashProvider.getLatSize(i)
            val lonSize = geohashProvider.getLonSize(i, point)
    ...
    fun filter(radius: Double, center: Point): List<StationPort> {
        ...
        val listGeohash = geohashProvider.getGeohashGridForPoint(center, level)
        for(geohash in listGeohash){
            val node = getNodeFromGeohash(geohash, level)
            if(node != null){
    ...
    override fun findStationsInsideCircle(radius: Double, point: Point): List<StationPort> {
        ...
        return geometryProvider.findPointsInsideCircle(
